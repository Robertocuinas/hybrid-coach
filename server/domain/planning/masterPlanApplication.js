...
export async function generateMasterPlanProposal(profileId, rawInput, deps = {}) {
  ...
  if (result.status !== "proposal" || !result.output) {
    ...
    console.error("[master-plan] NO PROPUESTA:", code, "fallback:", JSON.stringify(result.fallback || null).slice(0, 600),
      "evidence:", (result.evidence || []).length, "validation:", JSON.stringify(result.validation?.errors || result.validation?.guardrails?.hard || null).slice(0, 600));
